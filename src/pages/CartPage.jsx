...
import { DemoNotice, Quantity } from "../components/common";
...
export default function CartPage() {
  ...
  return (
    <>
      <Seo title="Shopping Cart" description="Review selected Yemi Hair Affordables demo products and variations before submitting an order request." path="/cart" />
          ...
          {!cart.length ? (
            ...
            <div className="cart-layout">
                ...
                {cart.map((item) => {
                  ...
                  return (
                    <article className="cart-item" key={item.key}>
                      <Link to={`/shop/${item.product.slug}`}><img src={item.product.images[0]} alt={`${item.product.name} demo style`} width="180" height="225" /></Link>
              ...
              <aside className="order-card">
                ...
                <div><span>Estimated delivery</span><strong>Confirmed later</strong></div>
                <label>Delivery or order note<textarea rows="3" placeholder="Optional note for Rosaline"></textarea></label>
                <label>Promo code <span>(placeholder)</span><div className="promo-row"><input disabled placeholder="Connect promotion system" /><button disabled type="button">Apply</button></div></label>
                <div className="order-card__total"><span>Demo total</span><strong>{formatMoney(cartTotal, currency)}</strong></div>
