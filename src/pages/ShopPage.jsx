  ...
  return (
    <div className="filters">
      ...
      <fieldset className="filter-group"><legend>Shop edit</legend>
        {[["featured", "Featured"], ["bestSeller", "Best sellers"], ["newArrival", "New arrivals"]].map(([key, label]) => <label key={key}><input type="checkbox" checked={filters[key]} onChange={() => setFilters({ ...filters, [key]: !filters[key] })} /><span>{label}</span></label>)}
      </fieldset>
...
export default function ShopPage() {
  ...
  const filtered = useMemo(() => {
    ...
    result = [...result].sort((a, b) => {
      if (sort === "price-low") return a.priceCAD - b.priceCAD;
      if (sort === "price-high") return b.priceCAD - a.priceCAD;
  ...
  return (
    <>
      <Seo title="Shop Wigs & Human Hair" description="Browse demo wigs and human-hair styles in bob, straight, curly, closure, frontal and headband collections with CAD and NGN pricing." path="/shop" />
      ...
      <div className="container shop-layout">
        ...
        <section className="shop-results" aria-labelledby="results-title">
          <div className="results-top"><h2 id="results-title">{filtered.length} {filtered.length === 1 ? "style" : "styles"}</h2><p>Demo catalogue · final details confirmed before fulfilment</p></div>
